...
export default function csfPlugin(svelteOptions) {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      const storyDef = Object.entries(stories).filter(([, def]) => !def.template).map(([storyId]) => `export const ${storyId} = __storiesMetaData.stories[${JSON.stringify(storyId)}];`).join('\n');
      s.replace('export default', '// export default');
      const namedExportsOrder = Object.entries(stories).filter(([, def]) => !def.template).map(([storyId]) => storyId);
      const output = ['', `import parser from '${parser}';`, `const __storiesMetaData = parser(${component}, ${JSON.stringify(all)});`, 'export default __storiesMetaData.meta;', `export const __namedExportsOrder = ${JSON.stringify(namedExportsOrder)};`, storyDef].join('\n');
      s.append(output);
      return {
        code: s.toString(),
        map: s.generateMap({
